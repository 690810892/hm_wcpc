...
//        SysInitInfo infor = hm_SfzcUserApplication.getInstance().getSysInitInfo();
//        String path = infor.getSys_web_service()+"webview/parm/feeinstruction";
//        in_webview.loadUrl(path);
//        out_webview.loadUrl(path);
    }

    private void getInfor(String keytype){
        String district = XtomSharedPreferencesUtil.get(mContext, "district");
        getNetWorker().feeRuleList("0", keytype, district);
    }
