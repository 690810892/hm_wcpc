 ...
 * 发布通知
 */
public class SendImageNoticeAdapter extends HemaAdapter implements OnClickListener {
	...
	public SendImageNoticeAdapter(Context mContext, View rootView,
	...
	private void setDataImage(int position, ViewHolder holder) {
		...
		if(path.indexOf("http") != -1){//包含http，说明是网上获取的，否则是本地图片地址
			ImageLoader.getInstance().displayImage(path, holder.imageView, hm_WcpcDriverApplication.getInstance().getOptions(R.mipmap.img_sendblog));
		}else{
			ImageLoader.getInstance().displayImage(ImageDownloader.Scheme.FILE.wrap(path),
					holder.imageView, hm_WcpcDriverApplication.getInstance().getOptions(R.mipmap.img_sendblog));
		}
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.button:
				TouSuActivity activity = (TouSuActivity) mContext;
